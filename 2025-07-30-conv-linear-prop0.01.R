...
make_torch_learner <- function(id,...){
  po_list <- list(
    ...
    mlr3pipelines::po(
      "torch_optimizer",
      mlr3torch::t_opt("sgd", lr=learning_rate)),
    mlr3pipelines::po(
      "torch_callbacks",
  ...
  mlr3tuning::auto_tuner(
    ...
    term_evals = 1,
    id=paste0(id,"_",train_loss,"_",learning_rate),
    store_models = TRUE)
}
n.pixels <- 28
n.epochs <- 10000
...
for(train_loss in names(mlr3torch_loss_list)){
  ...
  for(learning_rate in c(0.01, 0.1, 1)){
    arch.list <- list(
      make_torch_learner(
        "conv",
        mlr3pipelines::po(
...
proj.dir <- "/scratch/thocking/2025-07-30-conv-linear-prop0.01"
...
mlr3resampling::proj_test(proj.dir, max_jobs=1, min_samples_per_stratum = 10)

mlr3resampling::proj_submit(proj.dir, tasks=200, hours=30, gigabytes=16)
slurm::sjob()

res <- mlr3resampling::proj_results_save(proj.dir)
ltab <- fread(file.path(proj.dir, "learners.csv"))
mtab <- ltab[, names(ltab)[21:29],with=FALSE ]
fwrite(mtab, file.path(proj.dir, "learners_mini.csv"))
system(paste0("du -ms ", proj.dir, "/*csv"))
...
result.dt.list <- list()
rds.vec <- Sys.glob(file.path(proj.dir, "grid_jobs/*rds"))
rds.todo <- setdiff(rds.vec, names(result.dt.list))
for(job.i in seq_along(rds.todo)){
  job.rds <- rds.todo[[job.i]]
  cat(sprintf("%4d / %4d %s\n", job.i, length(rds.todo), job.rds))
  result.dt.list[[job.rds]] <- readRDS(job.rds)
}
...
lres <- fread(file.path(proj.dir, "learners.csv"))

(proj.dir <- "/scratch/thocking/2025-07-24-conv-linear-prop0.01")
(gjobs_c <- fread(file.path(proj.dir, "grid_jobs.csv")))
(gjobs_r <- readRDS(file.path(proj.dir, "grid_jobs.rds")))
gjobs_r[!gjobs_c, on=.NATURAL]
