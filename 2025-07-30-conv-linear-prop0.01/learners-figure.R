library(data.table)
## data files from rorqual:/scratch/thocking/2025-07-30-conv-linear-prop0.01
...
gg <- ggplot()+
  ...
  scale_y_log10(
    ...
    breaks=c(0.5, 10^seq(-10, -1)))
png("2025-07-30-conv-linear-prop0.01-AUM.png", width=20, height=10, units="in", res=200)
print(gg)
dev.off()

history_log <- history_show[!grepl("AUM", Learner) & train.subsets=="same"]
...
png("2025-07-30-conv-linear-prop0.01-logistic.png", width=20, height=10, units="in", res=200)
print(gg)
dev.off()

history_loss <- history_show[!grepl("AUM", Learner) & measure=="logloss" & train.subsets=="same"]
nc::capture_first_df(
  history_loss,
  learner_id=list(
    model=".*?",
...
gg <- ggplot()+
  ...
  scale_y_log10(
    ...
    breaks=c(0.5, 10^seq(-5, -1)))
png("2025-07-30-conv-linear-prop0.01-logistic-loss-MNIST.png", width=20, height=12, units="in", res=200)
print(gg)
dev.off()

history_loss <- history_show[!grepl("AUM", Learner) & task_id=="FashionMNIST_seed2_prop0.01"]
nc::capture_first_df(
  history_loss,
  learner_id=list(
    model=".*?",
    ...
    "_",
    lr="[0-9.]+", as.numeric))
min_dt <- history_loss[set=="validation", .SD[which.min(value)], by=.(Data, test.subset, train.subset, model, Measure)]
hline_dt <- min_dt[, .(Test_subset, Train_subset, Data, value, Measure)]
library(ggplot2)
gg <- ggplot()+
  ggtitle("Log loss, compare step sizes, FashionMNIST seed 2")+
  theme_bw()+
  geom_hline(aes(
    yintercept=value),
    data=hline_dt,
  ...
  geom_point(aes(
    ...
    data=data.frame(min_dt, point="min"))+
  scale_fill_manual(values=c(min="black"))+
  facet_grid(Train_subset + Test_subset + Measure ~ model + lr, labeller=label_both, scales="free")+
  scale_y_log10(
    "Objective value (log loss)",
    breaks=c(0.5, 10^seq(-5, -1)))
png("2025-07-30-conv-linear-prop0.01-logistic-loss-MNIST-zoom.png", width=15, height=8, units="in", res=200)
print(gg)
dev.off()
...
png("2025-07-30-conv-linear-prop0.01-AUM-compare-lr.png", width=20, height=22, units="in", res=200)
print(gg)
dev.off()
...
gg <- ggplot()+
  ...
  scale_y_log10(
    "Objective value (AUM)",
    breaks=c(0.5, 10^seq(-9, -1, by=2)))
png("2025-07-30-conv-linear-prop0.01-AUM-compare-lr-zoom.png", width=15, height=8, units="in", res=200)
print(gg)
dev.off()

history_loss <- history_show[task_id=="FashionMNIST_seed2_prop0.01" & test.subset=="balanced"]
nc::capture_first_df(
  history_loss,
  learner_id=list(
    model=".*?",
    ...
    "_",
    lr="[0-9.]+", as.numeric))
min_dt <- history_loss[set=="validation", .SD[which.min(value)], by=.(Data, test.subset, train.subset, model, Measure, loss)]
hline_dt <- min_dt[, .(Test_subset, Train_subset, Data, value, Measure, loss)]
library(ggplot2)
gg <- ggplot()+
  ggtitle("Log loss, compare step sizes, FashionMNIST seed 2, compare losses")+
  theme_bw()+
  geom_hline(aes(
    yintercept=value),
    data=hline_dt,
  ...
  geom_point(aes(
    ...
    data=data.frame(min_dt, point="min"))+
  scale_fill_manual(values=c(min="black"))+
  facet_grid(Train_subset + loss + Measure ~ model + lr, labeller=label_both, scales="free")+
  scale_y_log10(
    "Objective value",
    breaks=c(0.5, 10^seq(-9, -1, by=2)))
png("2025-07-30-conv-linear-prop0.01-compare-lr-zoom.png", width=15, height=8, units="in", res=200)
print(gg)
dev.off()

history_loss <- history_show[task_id=="FashionMNIST_seed2_prop0.01" & test.subset=="balanced" & measure=="invAUC"]
nc::capture_first_df(
  history_loss,
  learner_id=list(
    model=".*?",
    ...
    lr="[0-9.]+", as.numeric))
min_dt <- history_loss[set=="validation", .SD[which.min(value)], by=.(Data, test.subset, train.subset, model, Measure, loss)]
hline_dt <- min_dt[, .(Train_subset, model, value, Measure)]
library(ggplot2)
gg <- ggplot()+
  ggtitle("Log loss, compare step sizes, FashionMNIST seed 2, compare losses, invAUC")+
  theme_bw()+
  geom_hline(aes(
    yintercept=value),
    data=hline_dt,
  ...
  geom_point(aes(
    ...
    data=data.frame(min_dt, point="min"))+
  scale_fill_manual(values=c(min="black"))+
  facet_grid(Train_subset + model ~ loss + lr, labeller=label_both, scales="free")+
  scale_y_log10(
    "Objective value (inverse AUC)",
    breaks=c(0.5, 10^seq(-9, -1, by=2)))
png("2025-07-30-conv-linear-prop0.01-compare-lr-zoom-loss.png", width=15, height=8, units="in", res=200)
print(gg)
dev.off()
